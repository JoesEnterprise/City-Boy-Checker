...
class DB_Map():
    ...
    def create_graph(self, path, cities):
        """Create a map with marked cities"""
        ...
        for city in cities:
            ...
            if coords:
                lat, lng = coords
                ax.plot(lng, lat, 'r*', markersize=15, transform=ccrs.PlateCarree())
                ax.text(lng, lat, f'  {city}', transform=ccrs.PlateCarree(), fontsize=10)
    ...
    def draw_distance(self, city1, city2, path='map_distance.png'):
        """Draw a map with two cities and a line showing distance between them"""
        ...
        # Plot cities
        ax.plot(lng1, lat1, 'r*', markersize=15, transform=ccrs.PlateCarree())
        ax.plot(lng2, lat2, 'b*', markersize=15, transform=ccrs.PlateCarree())
        ax.text(lng1, lat1, f'  {city1}', transform=ccrs.PlateCarree(), fontsize=10)
        ax.text(lng2, lat2, f'  {city2}', transform=ccrs.PlateCarree(), fontsize=10)
        
        # Draw line between cities
        ax.plot([lng1, lng2], [lat1, lat2], 'g--', transform=ccrs.PlateCarree(), linewidth=2)
